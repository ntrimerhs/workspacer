...
return new Action<IConfigContext>((IConfigContext context) =>
{
    ...
    Action setKeybindings = () =>
    {
        ...
        // Left, Right keys
        manager.Subscribe(win, Keys.J, () => workspaces.SwitchToPreviousWorkspace(), "switch to previous workspace");
        manager.Subscribe(win, Keys.K, () => workspaces.SwitchToNextWorkspace(), "switch to next workspace");

        manager.Subscribe(winShift, Keys.H, () => workspaces.MoveFocusedWindowToPreviousMonitor(), "move focused window to previous monitor");
        manager.Subscribe(winShift, Keys.L, () => workspaces.MoveFocusedWindowToNextMonitor(), "move focused window to next monitor");

        // H, L keys
        manager.Subscribe(winCtrl, Keys.Left, () => workspaces.FocusedWorkspace.ShrinkPrimaryArea(), "shrink primary area");
        manager.Subscribe(winCtrl, Keys.Right, () => workspaces.FocusedWorkspace.ExpandPrimaryArea(), "expand primary area");

        // K, J keys
        manager.Subscribe(winShift, Keys.Left, () => workspaces.FocusedWorkspace.SwapFocusAndNextWindow(), "swap focus and next window");
        manager.Subscribe(winShift, Keys.Right, () => workspaces.FocusedWorkspace.SwapFocusAndPreviousWindow(), "swap focus and previous window");
        ...
        // Other shortcuts
        manager.Subscribe(winCtrl, Keys.D, () => actionMenu.ShowMenu(actionMenuBuilder), "show menu");
        manager.Subscribe(winShift, Keys.Escape, () => context.Enabled = !context.Enabled, "toggle enabled/disabled");
        manager.Subscribe(winShift, Keys.Q, () => context.ToggleConsoleWindow(), "toggle console window");
